...
class CorrBlock:
    ...
    def __call__(self, coords):
        ...
        for i in range(self.num_levels):
            ...
            delta = torch.stack(torch.meshgrid(dy, dx, dz), axis=-1)
    ...
    def corr(fmap1, fmap2):
        batch, dim, dp, ht, wd = fmap1.shape
        fmap1 = fmap1.view(batch, dim, dp * ht * wd)
        fmap2 = fmap2.view(batch, dim, dp * ht * wd)

        corr = torch.matmul(fmap1.transpose(1, 2), fmap2)
        corr = corr.view(batch, dp, ht, wd, 1, dp, ht, wd)
...
class SepConvGRU(nn.Module):
    def __init__(self, hidden_dim=128, input_dim=192 + 128):
        ...
        self.convz1 = nn.Conv3d(
            hidden_dim + input_dim, hidden_dim, (1, 5, 5), padding=(0, 2, 2)
        )
        ...
        self.convz2 = nn.Conv3d(
            hidden_dim + input_dim, hidden_dim, (5, 1, 5), padding=(2, 0, 2)
        )
        ...
        self.convz3 = nn.Conv3d(
            hidden_dim + input_dim, hidden_dim, (5, 5, 1), padding=(2, 2, 0)
        )
...
class BasicUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dim=128, input_dim=128):
        ...
        self.mask = nn.Sequential(
            ...
            nn.Conv3d(256, 8 * 8 * 8 * 27, 1, padding=0),
        )
...
def coords_grid(batch, dp, ht, wd, device):
...
default_raft_args = type("Args", (), default_raft_args)


class RAFT(nn.Module):
    def __init__(self, args=default_raft_args):
    ...
    def initialize_flow(self, img):
        ...
        N, C, D, H, W = img.shape
        coords0 = coords_grid(N, D // 8, H // 8, W // 8, device=img.device)
        coords1 = coords_grid(N, D // 8, H // 8, W // 8, device=img.device)
    ...
    def upsample_flow(self, flow, mask):
        ...
        up_flow = unfoldNd(8 * flow, [3, 3, 3], padding=1)
    ...
    def forward(
        ...
        for itr in range(iters):
            ...
            # interpolate flow
            flow = coords1 - coords0
            flow = self.interpolate_flow(flow, t)

            # upsample predictions
            if up_mask is None:
                flow_up = upflow8(flow)  # upflow8(coords1 - coords0)
            else:
                flow_up = self.upsample_flow(
                    flow, up_mask
                )  # self.upsample_flow(coords1 - coords0, up_mask)

            flow_predictions.append(flow_up)

        if test_mode:
            return flow, flow_up  # coords1 - coords0, flow_up

        return flow_predictions

    def interpolate_flow(self, flow, t):
        B, C, D, H, W = flow.size()
        t = t.repeat(1, 1, D, H, W)
